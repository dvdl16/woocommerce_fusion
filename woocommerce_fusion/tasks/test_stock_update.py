...
class TestWooCommerceOrder(FrappeTestCase):
	...
	def test_update_stock_levels_on_woocommerce_site(self, mock_wc_api, mock_frappe):
		...
		wc_additional_settings = frappe._dict(
			...
			warehouses=[
				frappe._dict(warehouse="Warehouse A", woocommerce_inventory_name="Storage A"),
				frappe._dict(warehouse="Warehouse B", woocommerce_inventory_name="Storage B"),
			],
		)
		mock_frappe.get_single.return_value = wc_additional_settings

		# Mock out calls to WooCommerce API's
		mock_product_inventories = [{"id": 123, "name": "Storage A"}, {"id": 345, "name": "Storage B"}]
		...
		actual_post_endpoints = [
			call.kwargs["endpoint"] for call in mock_api_instance.post.call_args_list
		]
		...
		expected_data = {
			"update": [
				{"id": 123, "meta_data": {"stock_quantity": 5}},
