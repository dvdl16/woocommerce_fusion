...
def get_shipment_providers(server):
	...
	all_providers = wc_api.get(f"orders/1/shipment-trackings/providers").json()
	if all_providers:
...
def get_woocommerce_shipment_providers(woocommerce_server_domain):
	...
	"""
	wc_settings = frappe.get_single(
		"WooCommerce Additional Settings"
	)
	return next((
		wc_server.wc_ast_shipment_providers for wc_server in wc_settings.servers
